...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        angle = 0;
        radius = self.frame.size.height/2 - _filledLineWidth/2 - 10;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetLineWidth(ctx, _unfilledLineWidth);
    ...
    CGContextSetLineWidth(ctx, _filledLineWidth);
    CGContextSetLineCap(ctx, kCGLineCapButt);
    CGContextDrawPath(ctx, kCGPathStroke);
...
-(void) drawHandle:(CGContextRef)ctx{
    ...
    if(_handleType == semiTransparentWhiteCircle) {
        ...
        CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x, handleCenter.y, _filledLineWidth, _filledLineWidth));
        ...
        [_handleColor set];
        CGContextAddArc(ctx, handleCenter.x + (_filledLineWidth)/2, handleCenter.y + (_filledLineWidth)/2, _filledLineWidth, 0, M_PI *2, 0);
        CGContextSetLineWidth(ctx, 7);
        CGContextSetLineCap(ctx, kCGLineCapButt);
        CGContextDrawPath(ctx, kCGPathStroke);
        CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x, handleCenter.y, _filledLineWidth-1, _filledLineWidth-1));
    } else if(_handleType == doubleCircleWithOpenCenter) {
        [_handleColor set];
        CGContextAddArc(ctx, handleCenter.x + (_filledLineWidth)/2, handleCenter.y + (_filledLineWidth)/2, 8, 0, M_PI *2, 0);
        CGContextSetLineWidth(ctx, 4);
        CGContextSetLineCap(ctx, kCGLineCapButt);
        CGContextDrawPath(ctx, kCGPathStroke);
        
        CGContextAddArc(ctx, handleCenter.x + _filledLineWidth/2, handleCenter.y + _filledLineWidth/2, _filledLineWidth/2, 0, M_PI *2, 0);
        CGContextSetLineWidth(ctx, 2);
        CGContextSetLineCap(ctx, kCGLineCapButt);
        CGContextDrawPath(ctx, kCGPathStroke);
    } else if(_handleType == bigCircle) {
        [_handleColor set];
        CGContextFillEllipseInRect(ctx, CGRectMake(handleCenter.x-2.5, handleCenter.y-2.5, _filledLineWidth+5, _filledLineWidth+5));
    }
...
-(CGPoint)pointFromAngle:(int)angleInt{
    ...
    CGPoint centerPoint = CGPointMake(self.frame.size.width/2 - _filledLineWidth/2, self.frame.size.height/2 - _filledLineWidth/2);
